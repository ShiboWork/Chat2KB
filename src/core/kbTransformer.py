...
class knowledgeTransformer(dsClient):
    ...
    def _build_system_prompt(self, knowledge):
        """构建知识提取专用提示词"""
        return f"""你是一个个人信息提炼专家，请严格从对话记录中提取用户特征信息。注意只需要提炼用户的信息。请按如下规则处理：
信息类型：个人特征与事件
   - 生活兴趣：如运动、音乐偏好、常参加的娱乐活动
   - 行为习惯：如晨跑、午睡、每周固定行程
   - 重要经历：如升学、就业、旅行、重大成就/挫折
   - 特定领域：表现出专业知识的领域（如编程、摄影）
   
1. 根据聊天长度生成合适个数重要的内容,最少一个，最多10个
